...
export default async function OrderPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const runtimeEnv = env as unknown as { PIX_KEY?: string; PIX_RECEIVER_NAME?: string; PIX_RECEIVER_CITY?: string; PIX_STATIC_PAYLOAD?: string };
  ...
  const pixPayload = quoteReady ? runtimeEnv.PIX_STATIC_PAYLOAD ?? (pixConfig ? createPixPayload(order.totalCents!, pixConfig, order.code) : null) : null;
  const usesStaticPixPayload = Boolean(runtimeEnv.PIX_STATIC_PAYLOAD);
